...
> **Note:** Hostinger requires `MAIL_SCHEME=smtps` (not `ssl`) for port 465. Passwords containing `#` must be quoted in `.env` (e.g. `IMAP_PASSWORD="#abc123"`), otherwise `#` is treated as a comment and the value is silently empty.
