@extends('layouts.app')

@section('title', 'Journal — The Workshop')
@section('meta_description', 'Stories, techniques, and inspiration from the Timber Trace Crafts studio.')

@section('content')

{{-- ============================================================ --}}
{{-- PAGE HEADER --}}
{{-- ============================================================ --}}
<div class="border-b border-walnut/20 py-12 md:py-16">
    <div class="page-container">
        <p class="section-label mb-4">The Workshop</p>
        <h1 class="font-heading text-5xl md:text-6xl font-light text-charcoal">Journal</h1>
    </div>
</div>

{{-- ============================================================ --}}
{{-- POSTS GRID --}}
{{-- ============================================================ --}}
<div class="page-container py-14 md:py-20">

    @if($posts->isEmpty())
        <div class="py-24 text-center">
            <p class="font-heading text-3xl font-light text-charcoal mb-4">No posts yet.</p>
            <p class="font-body text-sm text-walnut">Check back soon — we're always writing.</p>
        </div>
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 md:gap-12">
            @foreach($posts as $post)
                <article>
                    {{-- Image --}}
                    <a href="{{ route('journal.show', $post->slug) }}" class="block mb-5">
                        @if($post->featured_image_id && $post->featuredImage)
                            <picture>
                                <source srcset="{{ preg_replace('/\.(png|jpe?g)$/i', '.webp', $post->featuredImage?->url()) }}" type="image/webp">
                                <img src="{{ $post->featuredImage?->url() }}"
                                     alt="{{ $post->title }}"
                                     class="w-full aspect-video object-cover">
                            </picture>
                        @else
                            <div class="w-full aspect-video bg-walnut/10 flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="0.75" stroke="currentColor" class="w-10 h-10 text-walnut/25">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5z" />
                                </svg>
                            </div>
                        @endif
                    </a>

                    {{-- Date --}}
                    <p class="section-label mb-2">
                        {{ \Carbon\Carbon::parse($post->published_at)->format('F j, Y') }}
                    </p>

                    {{-- Title --}}
                    <h2 class="font-heading text-xl font-light text-charcoal leading-snug mb-3">
                        <a href="{{ route('journal.show', $post->slug) }}"
                           class="hover:text-forest-green transition-colors">
                            {{ $post->title }}
                        </a>
                    </h2>

                    {{-- Excerpt --}}
                    @if($post->excerpt)
                        <p class="font-body text-sm text-walnut leading-relaxed mb-4">
                            {{ Str::limit($post->excerpt, 130) }}
                        </p>
                    @endif

                    <a href="{{ route('journal.show', $post->slug) }}"
                       class="font-body text-xs tracking-widest uppercase border-b border-charcoal pb-0.5 hover:text-forest-green hover:border-forest-green transition-colors">
                        Read More
                    </a>
                </article>
            @endforeach
        </div>

        {{-- Pagination --}}
        @if($posts->hasPages())
            <div class="mt-16 pt-8 border-t border-walnut/20">
                {{ $posts->links() }}
            </div>
        @endif
    @endif

</div>

@endsection
