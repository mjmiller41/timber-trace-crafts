...
<div style="max-width: 480px;">
    ...
    <div class="admin-card">
        <div class="admin-card-header">
            <span class="admin-card-title">{{ $tag->name }}</span>
        </div>
        ...
        <form method="POST" action="{{ route('admin.tags.update', $tag) }}">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <select id="type" name="type" class="admin-input">
                    <option value="general" @selected(old('type', $tag->type) === 'general')>General</option>
                    <option value="wood_species" @selected(old('type', $tag->type) === 'wood_species')>Wood Species</option>
                    <option value="style" @selected(old('type', $tag->type) === 'style')>Style</option>
                </select>
            </div>
